...
<!-- source-map 顾名思义生成了从源代码到目标代码之间的双向映射，生成的代码某一行某一列可以通过 source-map 回溯到源代码的某一行和某一列；反过来，从源代码的某一行和某一列可以通过 source-map 跟踪到生成代码的某一行某一列。我们在调试器中通过生成代码去查看源代码是非常自然的需求，那么这里有一个思考题，为什么我们还要求从源代码能跟踪到生成代码呢？（设置断点）
!-->
